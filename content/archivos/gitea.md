...
``` 
 Y ahora si nos dirigimos en este caso sería a senchpimy.xyz:3000, deberíamos ver la página de instalación web, en el tipo de database escogí sqlite, pues solo estaré yo usando ese servidor y no hay necesidad de una database más robusta, en este apartado nos dará opción de modificar las cosas más importantes para que funcione, después de que ya esta configurado, el servidor ya es usable, en este caso solo desdel puerto **3000** y para cambiar esto hay que hacerlo desde **nginx (más información [aquí](./subdominios.html))**.
